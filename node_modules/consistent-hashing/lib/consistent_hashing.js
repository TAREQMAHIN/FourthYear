...
ConsistentHashing.prototype.getNodes = function() {
  return this.nodes;
};
...
ConsistentHashing.prototype.addNode = function(node) {
  ...
  for (var i = 0; i < this.replicas; i++) {
    var key = this.crypto(node.id || node);
...
// returns the index of the node in [nodes array] on which query should be processed
ConsistentHashing.prototype.getNodeIndex = function(key) {
  // console.log(this.ring);
  // console.log(this.keys);
  // console.log(this.nodes);
  if (this.getRingLength() == 0) return 0;
